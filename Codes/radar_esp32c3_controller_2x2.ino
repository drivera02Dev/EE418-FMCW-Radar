/*
 * FMCW MIMO RADAR — ESP32-C3 SPI & State Machine Controller
 * =================================================================
 * 1:1 port of the bench-validated Arduino Uno firmware to
 * ESP32-C3-DevKitC-02 v1.1. Behavior on the wire is byte-for-byte
 * identical: same 24-bit SPI framing, same R/W flag bit, same pulse
 * widths and inter-pulse timings, same register init sequence, same
 * scratchpad verify, same MIMO frame pattern.
 *
 * Differences from the Arduino version are limited to platform glue:
 *   - ESP32-C3 GPIO numbers instead of Uno digital pins
 *   - SPI.begin(SCK, MISO, MOSI, -1) explicit-pin form (FSPI bus)
 *   - delay(200) at boot in place of while(!Serial)
 *   - PIN_GPIO_DIR driven HIGH (new PCB uses 74AXP2T45 level shifters
 *     that require a DIR signal; the old TXB0108 setup did not)
 *
 * Hardware: ESP32-C3-DevKitC-02 v1.1 + Transmitter PCB (ADAR2001 +
 *           ADP7156 LDO + SN74LVC8T245 + 2x 74AXP2T45 level shifters)
 * SPI:      24-bit frame: [R/W(1)][ADDR(15)][DATA(8)], Mode 0,
 *           MSB first, 4 MHz on FSPI bus
 * Serial:   115200 baud over USB-UART (default UART0 = GPIO 20/21)
 *
 * SYNCHRONIZATION: Software sync (MATLAB orchestrates)
 *   MATLAB sends command -> ESP32-C3 executes -> "OK" -> MATLAB proceeds
 *   No hardware trigger wire needed.
 *
 * PIN MAPPING (ESP32-C3 -> PCB J1 logic header):
 *   GPIO  6 = SCK  (FSPI)         -> ADAR SCLK   (J1 pin 6)
 *   GPIO  7 = MOSI (FSPI)         -> ADAR SDIO   (J1 pin 7)
 *   GPIO  5 = MISO (FSPI)         <- ADAR SDO    (J1 pin 8)
 *   GPIO  4 = CS_TX               -> ADAR2001 CS (J1 pin 5)
 *   GPIO  8 = CS_RX               -> (TX-only board: not connected;
 *                                     this is also the onboard LED pin
 *                                     and a strapping pin, but stays
 *                                     HIGH the whole time so neither
 *                                     bothers anything)
 *   GPIO  0 = TxADV               -> ADAR2001 TxADV (J1 pin 1)
 *   GPIO  1 = TxRST               -> ADAR2001 TxRST (J1 pin 2)
 *   GPIO  3 = MADV                -> ADAR2001 MADV  (J1 pin 3)
 *   GPIO 10 = MRST                -> ADAR2001 MRST  (J1 pin 4)
 *   GPIO 18 = GPIO_DIR            -> 74AXP2T45 DIR  (J1 pin 9)
 *
 * AVOIDED PINS (ESP32-C3 specific):
 *   GPIO 11-17: reserved for internal SPI flash
 *   GPIO 20, 21: UART0 RX/TX (USB-UART for the serial monitor)
 *   GPIO 2, 9: strapping pins (boot-mode select, BOOT button) — left
 *              as inputs with their default pullups
 *
 * GPIO 18 and 19 are technically the C3's native USB D-/D+, but on
 * the DevKitC-02 v1.1 the USB connector is wired to the external
 * USB-UART chip (not the native USB), so these are free as GPIOs.
 *
 * POWER:
 *   PSU CH1 +5 V    -> J7 pin 1 (Vin), 500 mA limit
 *   PSU CH1 GND     -> J7 pin 3 (GND)
 *   ESP32-C3 3V3    -> J1 pin 10 (VLogic)
 *   ESP32-C3 GND    -> J1 pin 12 (GND)
 *
 * SERIAL COMMANDS (terminated by '\n'):
 *   I         — Init both ICs (full register programming)
 *   V         — Verify SPI (scratch-pad write -> readback -> compare)
 *   S         — Enable sequencers (hardware-pulse mode)
 *   D         — Disable sequencers (SPI-direct mode)
 *   P         — Pulse TxADV (advance TX channel, 50us settle included)
 *   R         — Reset state machines (MRST + TxRST)
 *   M         — Pulse MADV
 *   C N       — Set TX channel directly via SPI (1-4)
 *   W AAA DD  — Raw write to TX
 *   w AAA DD  — Raw write to RX
 *   X AAA     — Raw read from TX
 *   x AAA     — Raw read from RX
 *   ?         — Help
 */

#include <SPI.h>

// ===================== PIN DEFINITIONS =====================
// FSPI bus pins
#define PIN_SCK        6
#define PIN_MISO       5
#define PIN_MOSI       7

// CS lines
#define PIN_CS_TX      4
#define PIN_CS_RX      8

// State-machine control pins
#define PIN_TxADV      0
#define PIN_TxRST      1
#define PIN_MADV       3
#define PIN_MRST      10

// Level-shifter direction control
#define PIN_GPIO_DIR  18

#define SPI_SPEED  4000000
#define MULT_EN_16_20    0x6E
#define MULT_PASS_16_20  0x9F

// ===================== ACTIVE CHANNEL CONFIG =====================
// Which TX/RX channels are physically working. For 2x2 MIMO with damaged
// TX2/TX3 baluns and RFIN2/RFIN3 antennas, use {1, 4} for both.
// When all antennas are repaired, change to {1, 2, 3, 4}.
//
// This array drives:
//   - State-machine mode programming (initADAR2001)
//   - RX channel enable mask (initADAR2004)
//   - 'C N' command channel masks (setChannelDirect)
static const uint8_t TX_CHANNELS[] = {1, 4};
static const uint8_t RX_CHANNELS[] = {1, 4};
static const uint8_t NUM_TX_ACTIVE = sizeof(TX_CHANNELS) / sizeof(TX_CHANNELS[0]);
static const uint8_t NUM_RX_ACTIVE = sizeof(RX_CHANNELS) / sizeof(RX_CHANNELS[0]);

// TX channel masks for register 0x045 (TX_EN1_SPI). Index = channel - 1.
static const uint8_t TX_CH_MASK[4] = {0xC0, 0x30, 0x0C, 0x03};

// Helper: build RX_EN_SPI value (0x02B) from RX_CHANNELS array.
// Bits 7,6,5 = LNA, MIX, IFAMP enable. Bits 3..0 = CH1..CH4.
static uint8_t computeRxEnMask() {
  uint8_t mask = 0xE0;  // LNA + MIX + IFAMP all on
  for (uint8_t i = 0; i < NUM_RX_ACTIVE; i++) {
    uint8_t ch = RX_CHANNELS[i];
    if (ch >= 1 && ch <= 4) mask |= (0x08 >> (ch - 1));   // bit 3 = CH1, bit 2 = CH2, bit 1 = CH3, bit 0 = CH4
  }
  return mask;
}

// Helper: check if a TX channel is in the active set.
static bool isActiveTx(uint8_t ch) {
  for (uint8_t i = 0; i < NUM_TX_ACTIVE; i++) {
    if (TX_CHANNELS[i] == ch) return true;
  }
  return false;
}

// ===================== CORE SPI =====================

void spiWrite(uint8_t csPin, uint16_t addr, uint8_t data) {
  uint8_t b0 = (addr >> 8) & 0x7F;
  uint8_t b1 = addr & 0xFF;
  SPI.beginTransaction(SPISettings(SPI_SPEED, MSBFIRST, SPI_MODE0));
  digitalWrite(csPin, LOW);
  SPI.transfer(b0);
  SPI.transfer(b1);
  SPI.transfer(data);
  digitalWrite(csPin, HIGH);
  SPI.endTransaction();
}

uint8_t spiRead(uint8_t csPin, uint16_t addr) {
  uint8_t b0 = 0x80 | ((addr >> 8) & 0x7F);
  uint8_t b1 = addr & 0xFF;
  SPI.beginTransaction(SPISettings(SPI_SPEED, MSBFIRST, SPI_MODE0));
  digitalWrite(csPin, LOW);
  SPI.transfer(b0);
  SPI.transfer(b1);
  uint8_t result = SPI.transfer(0x00);
  digitalWrite(csPin, HIGH);
  SPI.endTransaction();
  return result;
}

// ===================== PULSE HELPERS =====================

void pulseTxADV() {
  digitalWrite(PIN_TxADV, HIGH);
  delayMicroseconds(1);
  digitalWrite(PIN_TxADV, LOW);
  delayMicroseconds(50);
}

void pulseTxRST() {
  digitalWrite(PIN_TxRST, HIGH);
  delayMicroseconds(1);
  digitalWrite(PIN_TxRST, LOW);
}

void pulseMADV() {
  digitalWrite(PIN_MADV, HIGH);
  delayMicroseconds(1);
  digitalWrite(PIN_MADV, LOW);
  delayMicroseconds(100);
}

void pulseMRST() {
  digitalWrite(PIN_MRST, HIGH);
  delayMicroseconds(1);
  digitalWrite(PIN_MRST, LOW);
}

// ===================== IC INIT =====================

bool verifySPI() {
  Serial.println(F("--- SPI Verification ---"));
  spiWrite(PIN_CS_TX, 0x00A, 0xA5);
  delay(1);
  uint8_t rd_tx = spiRead(PIN_CS_TX, 0x00A);
  bool tx_ok = (rd_tx == 0xA5);
  Serial.print(F("  TX: wrote 0xA5, read 0x")); Serial.print(rd_tx, HEX);
  Serial.println(tx_ok ? F(" PASS") : F(" FAIL"));

  spiWrite(PIN_CS_RX, 0x00A, 0x5A);
  delay(1);
  uint8_t rd_rx = spiRead(PIN_CS_RX, 0x00A);
  bool rx_ok = (rd_rx == 0x5A);
  Serial.print(F("  RX: wrote 0x5A, read 0x")); Serial.print(rd_rx, HEX);
  Serial.println(rx_ok ? F(" PASS") : F(" FAIL"));

  return tx_ok && rx_ok;
}

void initADAR2001() {
  Serial.println(F("--- Init ADAR2001 (TX) ---"));
  spiWrite(PIN_CS_TX, 0x000, 0x81); delay(10);
  spiWrite(PIN_CS_TX, 0x000, 0x18);
  spiWrite(PIN_CS_TX, 0x010, 0x01);
  spiWrite(PIN_CS_TX, 0x011, 0xBB);
  spiWrite(PIN_CS_TX, 0x012, 0x0B);
  spiWrite(PIN_CS_TX, 0x013, 0x75);
  spiWrite(PIN_CS_TX, 0x014, 0xB5);
  spiWrite(PIN_CS_TX, 0x015, 0x0C);
  spiWrite(PIN_CS_TX, 0x016, 0x10);
  spiWrite(PIN_CS_TX, 0x018, 0x10);
  spiWrite(PIN_CS_TX, 0x047, MULT_EN_16_20);
  spiWrite(PIN_CS_TX, 0x048, MULT_PASS_16_20);
  spiWrite(PIN_CS_TX, 0x045, 0xC0);
  spiWrite(PIN_CS_TX, 0x046, 0x07);
  // Mult modes
  spiWrite(PIN_CS_TX, 0x070, 0x00); spiWrite(PIN_CS_TX, 0x071, 0x00);
  spiWrite(PIN_CS_TX, 0x072, MULT_EN_16_20); spiWrite(PIN_CS_TX, 0x073, MULT_PASS_16_20);
  // TX modes — driven by TX_CHANNELS config at top of file.
  // Mode 0 = sleep. Modes 1..NUM_TX_ACTIVE are taken from TX_CHANNELS in order.
  // For 2-TX MIMO with TX_CHANNELS={1,4}: mode 1=TX1, mode 2=TX4 (states 3,4 unused).
  spiWrite(PIN_CS_TX, 0x050, 0x00); spiWrite(PIN_CS_TX, 0x051, 0x00);  // mode 0: sleep
  for (uint8_t i = 0; i < 4; i++) {
    uint16_t addr = 0x052 + i * 2;
    if (i < NUM_TX_ACTIVE) {
      spiWrite(PIN_CS_TX, addr,     TX_CH_MASK[TX_CHANNELS[i] - 1]);
      spiWrite(PIN_CS_TX, addr + 1, 0x07);
    } else {
      // Unused mode — leave as sleep (no PA active)
      spiWrite(PIN_CS_TX, addr,     0x00);
      spiWrite(PIN_CS_TX, addr + 1, 0x00);
    }
  }
  // State assignments — sequencer always cycles 0->1->2->3->4 mapped to modes 1,2,3,4.
  // Number of TxADV pulses per frame is controlled by NUM_TX_ACTIVE (in MATLAB or hw_trig).
  spiWrite(PIN_CS_TX, 0x03C, 0x10);
  spiWrite(PIN_CS_TX, 0x019, 0x12);
  spiWrite(PIN_CS_TX, 0x01A, 0x34);
  Serial.println(F("  Done."));
}

void initADAR2004() {
  Serial.println(F("--- Init ADAR2004 (RX) ---"));
  spiWrite(PIN_CS_RX, 0x000, 0x81); delay(10);
  spiWrite(PIN_CS_RX, 0x000, 0x18);
  spiWrite(PIN_CS_RX, 0x010, 0x01);
  spiWrite(PIN_CS_RX, 0x011, 0x55);
  spiWrite(PIN_CS_RX, 0x012, 0x07);
  spiWrite(PIN_CS_RX, 0x013, 0x78);
  spiWrite(PIN_CS_RX, 0x014, 0x7A);
  spiWrite(PIN_CS_RX, 0x015, 0x2A);
  spiWrite(PIN_CS_RX, 0x016, 0xC0);
  spiWrite(PIN_CS_RX, 0x017, 0x04);
  spiWrite(PIN_CS_RX, 0x018, 0x00);
  spiWrite(PIN_CS_RX, 0x019, 0x00);
  spiWrite(PIN_CS_RX, 0x02F, 0xEE);
  spiWrite(PIN_CS_RX, 0x02B, computeRxEnMask());
  spiWrite(PIN_CS_RX, 0x02E, 0x07);
  spiWrite(PIN_CS_RX, 0x02C, 0x77);
  spiWrite(PIN_CS_RX, 0x02D, 0x77);
  Serial.println(F("  Done."));
}

void enableSequencers() {
  spiWrite(PIN_CS_TX, 0x018, 0x90);
  spiWrite(PIN_CS_TX, 0x016, 0x90);
  spiWrite(PIN_CS_TX, 0x017, 0x03);
}

void disableSequencers() {
  spiWrite(PIN_CS_TX, 0x016, 0x10);
  spiWrite(PIN_CS_TX, 0x018, 0x10);
}

void setChannelDirect(uint8_t ch) {
  if (!isActiveTx(ch)) {
    Serial.print(F("WARN: TX")); Serial.print(ch);
    Serial.println(F(" not in active set, switching anyway"));
  }
  uint8_t val;
  switch (ch) {
    case 1: val = 0xC0; break; case 2: val = 0x30; break;
    case 3: val = 0x0C; break; case 4: val = 0x03; break;
    default: val = 0x00; break;
  }
  spiWrite(PIN_CS_TX, 0x045, val);
  spiWrite(PIN_CS_TX, 0x046, 0x07);
}

// ===================== SERIAL HANDLER =====================

void processCommand() {
  String cmd = Serial.readStringUntil('\n');
  cmd.trim();
  if (cmd.length() == 0) return;
  char c = cmd.charAt(0);

  switch (c) {
    case 'I':
      initADAR2001(); initADAR2004();
      Serial.println(F("OK INIT"));
      break;
    case 'V':
      Serial.println(verifySPI() ? F("OK VERIFY") : F("FAIL VERIFY"));
      break;
    case 'S':
      enableSequencers();
      Serial.println(F("OK SEQ_ON"));
      break;
    case 'D':
      disableSequencers();
      Serial.println(F("OK SEQ_OFF"));
      break;
    case 'P':
      pulseTxADV();
      Serial.println(F("OK TxADV"));
      break;
    case 'R':
      pulseMRST(); pulseTxRST(); delayMicroseconds(10);
      Serial.println(F("OK RESET"));
      break;
    case 'M':
      pulseMADV();
      Serial.println(F("OK MADV"));
      break;
    case 'C': {
      uint8_t ch = cmd.substring(2).toInt();
      if (ch >= 1 && ch <= 4) {
        setChannelDirect(ch);
        Serial.print(F("OK CH")); Serial.println(ch);
      } else Serial.println(F("ERR CH 1-4"));
      break;
    }
    case 'W': {
      uint16_t a = strtol(cmd.substring(2,5).c_str(), NULL, 16);
      uint8_t  d = strtol(cmd.substring(6,8).c_str(), NULL, 16);
      spiWrite(PIN_CS_TX, a, d);
      Serial.print(F("OK WR_TX 0x")); Serial.print(a,HEX);
      Serial.print(F("=0x")); Serial.println(d,HEX);
      break;
    }
    case 'w': {
      uint16_t a = strtol(cmd.substring(2,5).c_str(), NULL, 16);
      uint8_t  d = strtol(cmd.substring(6,8).c_str(), NULL, 16);
      spiWrite(PIN_CS_RX, a, d);
      Serial.print(F("OK WR_RX 0x")); Serial.print(a,HEX);
      Serial.print(F("=0x")); Serial.println(d,HEX);
      break;
    }
    case 'X': {
      uint16_t a = strtol(cmd.substring(2,5).c_str(), NULL, 16);
      uint8_t  v = spiRead(PIN_CS_TX, a);
      Serial.print(F("OK RD_TX 0x")); Serial.print(a,HEX);
      Serial.print(F("=0x")); Serial.println(v,HEX);
      break;
    }
    case 'x': {
      uint16_t a = strtol(cmd.substring(2,5).c_str(), NULL, 16);
      uint8_t  v = spiRead(PIN_CS_RX, a);
      Serial.print(F("OK RD_RX 0x")); Serial.print(a,HEX);
      Serial.print(F("=0x")); Serial.println(v,HEX);
      break;
    }
    case '?':
      Serial.println(F("=== FMCW MIMO Radar Controller ==="));
      Serial.println(F("I=Init V=Verify S=SeqOn D=SeqOff"));
      Serial.println(F("P=TxADV R=Reset M=MADV C N=SetCh"));
      Serial.println(F("W AAA DD=WriteTX w AAA DD=WriteRX"));
      Serial.println(F("X AAA=ReadTX x AAA=ReadRX"));
      Serial.println(F("MIMO: R->M->P,cap->P,cap->P,cap->P,cap"));
      break;
    default:
      Serial.print(F("ERR: ")); Serial.println(cmd);
      break;
  }
}

void setup() {
  Serial.begin(115200);
  delay(200);
  pinMode(PIN_CS_TX, OUTPUT); pinMode(PIN_CS_RX, OUTPUT);
  pinMode(PIN_TxADV, OUTPUT); pinMode(PIN_TxRST, OUTPUT);
  pinMode(PIN_MADV, OUTPUT);  pinMode(PIN_MRST, OUTPUT);
  pinMode(PIN_GPIO_DIR, OUTPUT);
  digitalWrite(PIN_CS_TX, HIGH); digitalWrite(PIN_CS_RX, HIGH);
  digitalWrite(PIN_TxADV, LOW);  digitalWrite(PIN_TxRST, LOW);
  digitalWrite(PIN_MADV, LOW);   digitalWrite(PIN_MRST, LOW);
  digitalWrite(PIN_GPIO_DIR, HIGH);
  SPI.begin(PIN_SCK, PIN_MISO, PIN_MOSI, -1);
  Serial.println(F(""));
  Serial.println(F("=== FMCW MIMO Radar Ready (ESP32-C3) ==="));
  Serial.println(F("HW SPI 4MHz, 24-bit, Mode 0"));
  Serial.print(F("Active TX: ["));
  for (uint8_t i = 0; i < NUM_TX_ACTIVE; i++) {
    Serial.print(TX_CHANNELS[i]); if (i < NUM_TX_ACTIVE - 1) Serial.print(F(","));
  }
  Serial.print(F("]  Active RX: ["));
  for (uint8_t i = 0; i < NUM_RX_ACTIVE; i++) {
    Serial.print(RX_CHANNELS[i]); if (i < NUM_RX_ACTIVE - 1) Serial.print(F(","));
  }
  Serial.println(F("]"));
  Serial.println(F("Software sync — send ? for help"));
}

void loop() {
  if (Serial.available()) processCommand();
}
