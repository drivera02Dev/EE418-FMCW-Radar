...
//
// Effects:
//   - State-machine modes 1..NUM_TX_ACTIVE programmed from TX_CHANNELS
//   - doMIMOFrame loops NUM_TX_ACTIVE times (not always 4)
//   - RX_EN_SPI mask computed from RX_CHANNELS
...
void initADAR2001() {
  ...
  // State assignments — sequencer cycles 0->1->2->3->4 mapped to modes 1..4.
  // doMIMOFrame controls how many TxADV pulses fire per frame.
...
void doMIMOFrame() {
  ...
  // Loop over active TX channels (configured at top of file)
  for (uint8_t ch = 0; ch < NUM_TX_ACTIVE; ch++) {
...
void setup() {
  ...
  Serial.print(NUM_TX_ACTIVE); Serial.println(F(" chirps"));
